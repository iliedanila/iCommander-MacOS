...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        let container = NSPersistentContainer(name: "iCommander")
        container.loadPersistentStores(completionHandler: { (storeDescription, error) in
            if let error = error {
                // Log the error for debugging
                NSLog("Core Data error: \(error.localizedDescription)")
                
                // Present error to user instead of crashing
                DispatchQueue.main.async {
                    let alert = NSAlert()
                    alert.messageText = "Database Error"
                    alert.informativeText = "The application encountered an error loading its data. The app may not function correctly. Error: \(error.localizedDescription)"
